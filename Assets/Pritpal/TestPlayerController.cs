...
public class TestPlayerController : MonoBehaviour, IDamageTaker, IRewardTaker, IAbilityTaker
{
   //variables needed for moving
    ...
    [SerializeField] private GameObject p2Head;
     [SerializeField] AudioManager _audioMan;
    [SerializeField] AudioSource _audioSource;
    //player animator for animations
    [SerializeField] private Animator _animator;
   
   [SerializeField] InvincibilityUI powerUI;
    ...
    public void TakeDamage(int damage) {  
        if( health > 3) {
            ...
            StartCoroutine(ResetHealth(3, 5f));
     ...
     private IEnumerator ResetHealth(int healthReset, float duration)
    {
        
         // Increase health by the power-up value
         var sliderObj = powerUI.transform.GetChild(0).GetChild(0).Find("Slider");
         if(sliderObj) Debug.Log("Found!!!");
         ...
         while (timeLeft > 0)
        {
            ...
            timeLeft -= Time.deltaTime * (timeFactor/duration);
            // Decrease the remaining time
            Debug.Log(timeLeft);
           // Update the slider based on remaining time
             
            yield return null; // Wait until the next frame
        }
        
        if(slider.value< 1) Health = healthReset;
        // Wait for the specified duration
        powerUI.transform.GetChild(0).gameObject.SetActive(false);

        // Reset health to maxHealth after the duration
    ...
    public void SetHeight(float a) => _jumpHeight = a;
    ...
    private void Awake()
    {
        ...
        _characterController = GetComponent<CharacterController>();
         _audioMan = GameObject.FindWithTag("AudioManager").GetComponent<AudioManager>();
        ...
        _tongueAttackCollider.center = new Vector3(_tongueAttackCollider.height / 2, 0, 0);
         powerUI = GameObject.FindWithTag("PowerUp").GetComponent<InvincibilityUI>();
    ...
    private void Jump()
    {
        ...
        if (_isJumpPressed && _isGrounded)
        {
            ...
            _velocity.y += Mathf.Sqrt(_jumpHeight * _gravity);
             if(_jumpHeight > _player1JumpHeight) SetHeight(_player1JumpHeight);
